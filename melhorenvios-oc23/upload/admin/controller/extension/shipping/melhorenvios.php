...
class ControllerExtensionShippingMelhorenvios extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/shipping/melhorenvios', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/shipping/melhorenvios', 'token=' . $this->session->data['token'], true);
    ...
    $headers = array('Accept: application/json', 'Content-Type: application/json;charset=UTF-8', 'User-Agent: Opencart Master suporte@opencartmaster.com.br');
    $soap_do = curl_init();
